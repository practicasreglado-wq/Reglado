...
window.iniciarFiltrosUI = function() {
    inicializarFiltros();
    ...
    if (toggleLeyenda && panelLeyenda) {
        // Prevenir duplicidad de eventos si Vue re-inyecta el script sin limpiar la UI (though UI is cleaned)
        toggleLeyenda.onclick = function(e) {
            e.stopPropagation();
...
window.iniciarDeepLinking = function(energiaForzada) {
    const urlParams = new URLSearchParams(window.location.search);
    const energiaURL = energiaForzada || urlParams.get('energia')?.toLowerCase();

    if (energiaURL) {
        console.log("🛠️ Procesando enlace para:", energiaURL);

        // ⏳ 1. Esperar a que la data esté cargada en memoria
