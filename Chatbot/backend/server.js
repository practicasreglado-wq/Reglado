...
import { crearArchivoNotion } from './services/notionService.js';
...
// Inicializar Telegram después de que app y express.json() estén listos.
// En modo webhook, necesita registrar la ruta POST /webhook/telegram en Express.
// En modo polling, solo necesita el token (no usa app).
...
// Caché de contenido web por dominio { domain: { texto, timestamp } }
const webCache = {};
const WEB_CACHE_TTL = 60 * 60 * 1000; // refresca cada hora

async function obtenerTextoWeb(domain) {
  if (!domain || domain === 'desconocido' || domain === 'localhost' || domain === '127.0.0.1') return null;

  const ahora = Date.now();
  if (webCache[domain] && (ahora - webCache[domain].timestamp) < WEB_CACHE_TTL) {
    return webCache[domain].texto;
  }
  ...
  try {
    ...
    webCache[domain] = { texto, timestamp: ahora };
    console.log(`[WebScraper] Contenido cacheado para ${domain} (${texto.length} chars)`);
    return texto;
  } catch (err) {
    console.warn(`[WebScraper] No se pudo obtener contenido de ${domain}:`, err.message);
    return null;
  }
}

// system prompt — acepta un resumen de memoria opcional para inyectar contexto de conversaciones previas
function buildSystemPrompt(domain, memoriaResumen = null, textoWeb = null) {
  const ahora = new Date();
  const fecha = ahora.toLocaleDateString('es-ES', { timeZone: 'Europe/Madrid', weekday: 'long', year: 'numeric', month: '2-digit', day: '2-digit' });
  const hora  = ahora.toLocaleTimeString('es-ES', { timeZone: 'Europe/Madrid', hour: '2-digit', minute: '2-digit', hour12: false });

  const partes = [
    'Eres el asistente virtual de Grupo Reglado. Tu prioridad es ayudar al cliente respondiendo sus preguntas de forma natural, amable y útil en español (salvo si preguntan en otro idioma). Usa iconos ocasionalmente.',
    `Fecha y hora actual (España): ${fecha}, ${hora}. Úsala siempre para validar horarios y disponibilidad de citas.`,
    `Dominio actual: ${domain || 'desconocido'}. Adapta tus respuestas al contexto de esa web y los servicios que ofrece.`,
    textoWeb ? `CONTENIDO DE LA WEB (${domain}): ${textoWeb}` : '',
    'COMPORTAMIENTO: Responde primero a lo que te pregunten de forma completa y útil. No ofrezcas citas ni agentes de forma proactiva ni repetitiva a menos que el cliente no pueda resolver su duda contigo, lo pida explícitamente, o sea claramente lo que necesita.',
    'HORARIO de citas y agentes humanos: Lunes a Viernes, 07:00-15:00.',
    'CITAS: Cuando el cliente quiera agendar una llamada, usa "agendar_llamada". Necesitas: nombre, email, móvil, día (DD-MM-YYYY) y hora (dentro del horario laboral).',
    'AGENTES: Usa "solicitar_agente_humano" si el cliente pide hablar con una persona o su consulta es demasiado compleja. NUNCA pidas datos personales antes de conectar — llama al tool directamente.',
    'ARCHIVOS: Si el cliente quiere subir un archivo: (1) Si ya tienes nombre, email y teléfono, llama a "registrar_usuario" inmediatamente. (2) Si no los tienes, pídelos y luego llama al tool. NUNCA respondas solo con texto cuando tengas que registrar.',
  ];
...
app.post('/api/upload', uploadLimiter, upload.single('archivo'), async (req, res) => {
  try {
    ...
    crearArchivoNotion({ nombre, email, mimetype: req.file.mimetype });
  } catch (error) {
...
app.post('/chat', chatLimiter, async (req, res) => {
  try {
    ...
    // Obtener contenido de la web del cliente (cacheado, solo se scrapea una vez por hora)
    const textoWeb = await obtenerTextoWeb(domain);
    ...
    const response = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 700,
      system: buildSystemPrompt(domain, memoriaResumen, textoWeb),
      tools: CHATBOT_TOOLS,
