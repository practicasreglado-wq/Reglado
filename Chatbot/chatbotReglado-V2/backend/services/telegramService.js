...
/**
 * Inicializa el bot de Telegram en modo polling (local) o webhook (producción).
 *
 * @param {import('express').Application} app - Instancia de Express (necesaria en modo webhook)
 */
export function initTelegramService(app) {
  if (!token) return;
  if (bot) return;
  ...
  if (webhookUrl) {
    ...
    // Telegram enviará cada update haciendo POST a nuestra URL.
    // No hay polling, no hay conflictos 409, y funciona aunque el proceso se reinicie.
    bot = new TelegramBot(token, { polling: false });

    // Registrar la URL del webhook en Telegram para que sepa dónde enviarnos los mensajes
    bot.setWebHook(webhookUrl)
      .then(() => console.log(`[TelegramService] Webhook registrado en Telegram: ${webhookUrl}`))
      .catch(err => console.error('[TelegramService] Error al registrar webhook:', err.message));

    // Ruta Express que recibe los updates que Telegram nos envía via POST
    app.post('/webhook/telegram', (req, res) => {
      bot.processUpdate(req.body); // Procesa el update igual que en polling
      res.sendStatus(200);         // Telegram espera un 200 rápido, si no reintenta
    });

    console.log('[TelegramService] Bot iniciado en modo WEBHOOK.');
  } else {
    // ── MODO POLLING (desarrollo local) ───────────────────────────────────────
    // El bot consulta a Telegram cada pocos segundos si hay mensajes nuevos.
    // IMPORTANTE: solo puede haber un proceso activo. Si hay dos, Telegram devuelve
    // error 409 Conflict. Antes de arrancar en local: taskkill /F /IM node.exe
    bot = new TelegramBot(token, { polling: true });

    // Eliminar cualquier webhook registrado en sesiones anteriores.
    // Si dejáramos un webhook activo, Telegram enviaría los mensajes allí
    // y el polling nunca recibiría nada.
    bot.deleteWebHook()
      .catch(() => {}); // Silenciar error si no había webhook registrado

    // Capturar el error 409 para que no spamee la consola.
    // Ocurre cuando otra instancia (local u otro servidor) ya está haciendo polling.
    // Solución: asegúrate de que solo hay un proceso activo con este token.
    bot.on('polling_error', (err) => {
      if (err.code === 'ETELEGRAM' && err.message.includes('409')) {
        console.warn('[TelegramService] 409 Conflict: hay otra instancia activa con este token. Detén el otro proceso o la app en Hostinger.');
      } else {
        console.error('[TelegramService] Error de polling:', err.message);
      }
    });

    console.log('[TelegramService] Bot iniciado en modo POLLING (local).');
  }
